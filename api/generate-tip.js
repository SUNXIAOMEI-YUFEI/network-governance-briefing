...
4. **加粗规则**（用 Markdown **xxx** 语法；前端会渲染成粗体）
   - 全文加粗 **6-10 处**——既要让读者扫一眼就抓到核心，又不能滥
   - **必须加粗**以下四类：
     a. **每段开篇的核心判断短语**（不是整句，是判断的关键 4-12 字。
        例："欧盟 AI 法案进入落地执行的关键阶段"里加粗 **进入落地执行**；
        "全球 AI 治理跨越一个分水岭"里加粗 **跨越一个分水岭**）。
        全文 1-2 段，因此这种判断短语加粗共 1-2 次
     b. **关键术语首次出现**（如 **数据保护影响评估**、**显著风险阈值**、
        **避风港规则**、**风险分级**、**透明度义务**）——加粗 2-4 处
     c. **关键数字事实**（如 **超过 85 万加州居民**、**违法所得 5 倍**、
        **逾 200 家平台**、**6 月下旬**）——加粗 1-3 处
     d. **并列序号词**（**一是**、**二是**、**三是**、**其一**、**其次**、**最后**）
        ——这是用户视觉锚点，不能省
   - **严禁加粗**：
     × 整句判断（一整句话从头加粗到尾——这是 AI 味标志）
     × 章节标签（~~**核心观点：**~~、~~**第一、**~~、~~**核心结论：**~~）——
       这是 prompt 痕迹，会让稿子像 ChatGPT 写的
     × 连接词（~~**事实上**~~、~~**整体来看**~~、~~**值得注意的是**~~）
