...
2. **段落结构**：1-2 段，每段 200-400 字，论证型长段，不是金句型短段
   ...
   - 用"一是 / 二是 / 三是"或"其一 / 其次 / 最后"嵌入长句串联，**不分行**
     （这些序号词本身可以加粗，详见第 4 条）

3. **数字、引语**：自然嵌入论述，不单独成行
   （例："超过85万加州居民的基因数据被泄露"嵌进句中作为论据）

4. **加粗规则**（用 Markdown **xxx** 语法；前端会渲染成粗体，方便屏幕阅读）
   - 全文加粗 **3-5 处**，不能更多
   - 只允许加粗以下三类，**其他一律不加粗**：
     a. 关键术语首次出现（如 **数据保护影响评估**、**显著风险阈值**、**避风港规则**）
     b. 关键数字事实（如 **超过 85 万加州居民**、**违法所得 5 倍**、**逾 200 家平台**）
     c. 并列序号词（**一是**、**二是**、**三是**、**其一**、**其次**、**最后**）
   - 严禁加粗：
     × 整句判断（如 ~~**全球 AI 治理正在重新洗牌**~~）
     × 章节标签（如 ~~**核心观点：**~~、~~**第一、**~~、~~**核心结论：**~~）
     × 连接词（如 ~~**事实上**~~、~~**整体来看**~~、~~**值得注意的是**~~）
